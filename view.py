...
for data, name in dataset:
    ...
    lane = input_data['map'][0, 3]
    sin = input_data['map'][0, 4]
    cos = input_data['map'][0, 5]
    map_layers = np.stack([sin] * 3).transpose(1, 2, 0)
    ...
    fig.savefig(f'./view/{name}(sin).png')
    plt.close(fig)
    ...
    sin = input_data['map'][0, 4]
    cos = input_data['map'][0, 5]
    map_layers = np.stack([cos] * 3).transpose(1, 2, 0)
    ax.imshow(map_layers, extent=[-axes_limit, axes_limit, -axes_limit, axes_limit])
    for i in range(length.item()):
        if input_data['category'][0, i] != 0:
            color = cat2color[input_data['category'][0, i].item()]
    ...
    ax.set_xlim([-axes_limit, axes_limit])
    ax.set_ylim([-axes_limit, axes_limit])
    fig.savefig(f'./view/{name}(cos).png')
    plt.close(fig)
